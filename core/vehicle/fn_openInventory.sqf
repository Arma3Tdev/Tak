...
if(isNull _vehicle OR !(_vehicle isKindOf "Car" OR _vehicle isKindOf "Air" OR _vehicle isKindOf "Ship" OR _vehicle isKindOf "Land_House_K")) exitWith {}; //Either a null or invalid vehicle type.
...
if(_vehicle isKindOf "Land_House_K") then {
...
if(_vehicle isKindOf "Land_House_K" && {count (_vehicle getVariable ["containers",[]]) == 0}) exitWith {hint localize "STR_MISC_NoStorageWarn"; closeDialog 0; _vehicle setVariable["trunk_in_use",false,true];};
if(_veh_data select 0 == -1 && {!(_vehicle isKindOf "Land_House_K")}) exitWith {closeDialog 0; _vehicle setVariable["trunk_in_use",false,true]; hint localize "STR_MISC_NoStorageVeh";};
...
_vehicle spawn
{
	...
	if(_this isKindOf "Land_House_K") then {
